defmodule SymphonyElixir.Config.Schema do
  ...
  @spec normalize_dispatch_kinds(term()) :: [String.t()]
  def normalize_dispatch_kinds(kinds) when is_list(kinds) do
    kinds
    |> Enum.map(&normalize_dispatch_kind/1)
    |> Enum.reject(&is_nil/1)
    |> Enum.uniq()
  end

  def normalize_dispatch_kinds(_kinds), do: []

  @spec normalize_dispatch_kind(term()) :: String.t() | nil
  def normalize_dispatch_kind(nil), do: "issue"

  def normalize_dispatch_kind(kind) when is_atom(kind) do
    kind
    |> Atom.to_string()
    |> normalize_dispatch_kind()
  end

  def normalize_dispatch_kind(kind) when is_binary(kind) do
    normalized =
      kind
      |> String.trim()
      |> String.downcase()
  ...
  def normalize_dispatch_kind(_kind), do: nil

  @spec validate_dispatch_kinds(Ecto.Changeset.t(), atom()) :: Ecto.Changeset.t()
  def validate_dispatch_kinds(changeset, field) do
    validate_change(changeset, field, fn ^field, kinds ->
      invalid_kinds = Enum.reject(kinds, &(&1 in ["issue", "pull_request"]))

      cond do
        kinds == [] ->
          [{field, "must include at least one kind"}]

        invalid_kinds != [] ->
          [{field, "must contain only issue or pull_request"}]
