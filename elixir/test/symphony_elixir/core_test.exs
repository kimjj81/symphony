defmodule SymphonyElixir.CoreTest do
  ...
  test "current WORKFLOW.myven.md keeps responsive webhook fallback settings" do
    ...
    assert prompt =~ "Todo items are planning-only"
    assert prompt =~ "Planned and In Progress items are implementation lanes"
  ...
  defp assert_due_in_range(due_at_ms, window_started_at_ms, min_delay_ms, max_delay_ms) do
    observed_at_ms = System.monotonic_time(:millisecond)

    assert due_at_ms >= window_started_at_ms + min_delay_ms
