...
21. Symphony's broker owns the orchestration snapshot. Before every fresh worker turn, it reads the current work item, fully paginated general comments, review summaries, inline comments, and unresolved provider threads using broker credentials, then emits a new 8 KiB compact manifest and indexed YAML evidence sidecar capped at 8 MiB. Workers MUST read the index and every lane-required region, MAY read optional regions when context is insufficient, MUST treat the sidecar as immutable, and MUST NOT query GitHub. This evidence MUST NOT redefine the worker's execution lane, write authority, allowed scope, tracker transition, or semantic outcome.
